...
uint8_t txData[256];    //TX data buffer. Could be up to 256 bytes.
...
void setup() 
{
  ...
  uint32_t netToken = 0x0;  //Insert your network token. For example, the OPEN network token is 0x4f50454e.
                                   //A module can only talk to gateways using the same network token.
  //Set desired application token
  uint8_t appToken[APP_TOKEN_LEN] = {0x0}; //Generate an application token in your Conductor account use it here.
